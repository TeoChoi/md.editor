...
class MdEditor {
    ...
    constructor(id, opts) {
        ...
        let panel, dialog, commandManager, undoManager, format, previewManager;

        panel = new Panel(this.id);
        dialog = new Dialog(options.uploadUrl);

        commandManager = new CommandManager(this.getString);
        commandManager.dialog = dialog;
        undoManager = new UndoManager(panel.input);
        format = new Format(panel.input, options.converter);

        if (options.preview) {
            previewManager = new PreviewManager(panel, format);
